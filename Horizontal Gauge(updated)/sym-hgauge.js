(function (PV) {  
    ...
    var definition = {  
        typeName: 'hgauge',  
        displayName : 'C8 Horizontal Gauge',
        datasourceBehavior: PV.Extensibility.Enums.DatasourceBehaviors.Single,  
        iconUrl: 'Images/chrome.gauge_horizontal_new.svg', 
    ...
    symbolVis.prototype.init = function init(scope, elem) { 
        ...
        scope.containerId = 'horizontal-gauge-' + Math.ceil(Math.random() * 10000000)
        ...
        function loadChart(data) {
             ...
             gauge.background(scope.config.BackgroundFill);
            ...
            labels.vAlign("bottom");
            labels.offsetX("10");
            labels.offsetY(30);
            labels.position("outside");
            labels.format(parseFloat(pointVal) + (scope.config.ShowUOM ? scope.config.ValueUnit : ''));
             ...
             // format axis labels
             axis.labels().format('{%value}' + (scope.config.IsPercent ? '%' : ''));
             axis.labels().fontSize(9);
             axis.labels().fontColor(scope.config.OutlineColor);

             // set paddings
             gauge.padding([0, 20, 10]);
